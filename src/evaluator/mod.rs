...
impl Evaluator {
    ...
    pub fn eval(&mut self, node: Option<&dyn Node>) -> Option<Rc<dyn Object>> {
        if let Some(node) = node {
            if let Some(program) = node.as_any().downcast_ref::<Program>() {
                Some(self.eval_statements(&program.statements))
            } else if let Some(stmt) = node.as_any().downcast_ref::<ExpressionStatement>() {
                match &stmt.expression {
                    Some(expr) => self.eval(Some(expr.as_ref())),
                    None => None,
                }
    ...
    fn eval_bang_operator(&mut self, right: Rc<dyn Object>) -> Option<Rc<dyn Object>> {
    ...
    fn eval_minus_prefix_operator(&mut self, right: Rc<dyn Object>) -> Option<Rc<dyn Object>> {
    ...
    fn eval_statements(&mut self, stmts: &[Box<dyn Statement>]) -> Rc<dyn Object> {
        let mut result = None;
        for stmt in stmts.iter() {
            result = self.eval(Some(stmt.as_ref()));
    ...
    fn eval_if_expression(&mut self, if_expr: &IfExpr) -> Option<Rc<dyn Object>> {
        let condition = self.eval(Some(if_expr.condition.as_ref()));
                ...
                Some(condition) => {
                    if *condition.val.borrow_mut() {
                        if let Some(ref consequence) = if_expr.consequence {
                            self.eval(Some(consequence.as_ref()))
                        } else {
                            None
                        }
                    } else if let Some(ref el_if) = if_expr.el_if {
                        self.eval(Some(el_if.as_ref()))
                    } else if let Some(ref alternative) = if_expr.alternative {
                        self.eval(Some(alternative.as_ref()))
                    } else {
                        None
                    }
                }
    ...
    fn eval_ret_stmt(&mut self, ret: &Return) -> Option<Rc<dyn Object>> {
        match &ret.return_value {
            Some(expr) => {
                let val = self.eval(Some(expr.as_ref()));
                match val {
    ...
    fn is_error(&mut self, to_check: &Option<Rc<dyn Object>>) -> bool {
        matches!(to_check, Some(check) if check.get_type() == Type::Error)
    }

    fn eval_identifier(&mut self, identifier: &Identifier) -> Option<Rc<dyn Object>> {
        let val = self.get(identifier.to_string());
        if val.is_some() {
            val
        } else {
            Some(Rc::new(Error::new(format!(
